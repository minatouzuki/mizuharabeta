...
async def ping(c: Mizuhara, m: Message):
    first = datetime.now()
...
async def get_lyrics(c: Mizuhara, m: Message):
...
async def id_info(c: Mizuhara, m: Message):
...
async def get_gifid(c: Mizuhara, m: Message):
...
async def my_info(c: Mizuhara, m: Message):
...
@Mizuhara.on_message(filters.command("weebify", PREFIX_HANDLER))
async def weebify(c: Mizuhara, m: Message):
